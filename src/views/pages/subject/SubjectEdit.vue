...
const getSingleSubjectData = async() =>{
    const res = await commonStores.singleData(`/admin/subjects/${route.params.id}`);
    if(res?.success){
        name.value     = res?.result?.name;
        status.value   = res?.result?.status;
        itemCode.value = res?.result?.item_code;
        unite.value    = res?.result?.unite;
        stock.value    = res?.result?.stock;
        price.value    = res?.result?.unite_price;
    }
}
...
const submit = async() =>{
    ...
    const res = await commonStores.update(`/admin/subjects/${route.params.id}`, formData);
    if(res?.success){
            ...
            <div class="col-md-12 add-permission-area">
                <div class="card p-0">
                    <div class="row p-4">
                        ...
                        <div class="col-md-6">
                            <div class="form-group mb-4">
                                <label for="number">Unite <span class="text-danger"> * </span></label>
                        ...
                        <div class="col-md-6">
                            <div class="form-group mb-4">
                                ...
                                <input type="text" id="stock" placeholder="Stock " class="form-control" v-model="stock">
                                <span class="errors">
                        ...
                        <div class="col-md-12">
                            <div class="d-flex justify-content-end">
                                ...
                                <button class="btn btn-add" @click="submit" v-else><i class="fa-solid fa-plus"></i> Update </button>
                            </div>
                        </div>
                    </div>
                </div>
