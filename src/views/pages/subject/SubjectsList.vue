    ...
    Swal.fire({
        ...
        (async () => {
            if (result.isConfirmed) {
                const res = await commonStores.destroy(`/admin/subjects/${removeId}`);
                if (res.success) {
                                ...
                                <table class="table table-striped table-hover permission-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="width:15%;">Unite Price</th>
                                            <th style="width:15%;">Status</th>
                                            <th style="width:15%;">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr v-for="(value, index) in subjects?.data" :key="index">
                                            <td>{{ value.item_code }}</td>
                                            <td>{{ value.name }}</td>
                                            <td>{{ value.unite }}</td>
                                            <td>{{ value.stock }}</td>
                                            <td>{{ value.unite_price }}</td>
                                            <td>{{ value.status }}</td>
                                            <td>
                                                <router-link :to="{name:'item-edit', params : {id: value.id} }" class="btn btn-approve btn-sm"><i class="fa-solid fa-pen-to-square"></i></router-link>
                                                <a class="btn btn-remove btn-sm ms-2" @click="destroy(value.id)" ><i class="fa-solid fa-trash"></i></a>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
